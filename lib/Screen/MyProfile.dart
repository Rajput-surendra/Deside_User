...
import 'package:entemarket_user/Screen/custom.dart';
...
class StateProfile extends State<MyProfile> with TickerProviderStateMixin {
  ...
  Future<void> setUpdateUser(String userID,
      [oldPwd, newPwd, username, userEmail,userPhone]) async {
    ...
    if (!error) {
      ...
      if ((username != "") && (userEmail != "")) {
        ...
        settingProvider.setPrefrence(MOBILE, userPhone);
        userProvider.setMobile(userPhone);
      }
  ...
  _getDrawer() {
    return ListView(
      ...
      children: <Widget>[
        ...
        CUR_USERID == "" || CUR_USERID == null
            ? Container()
            : _getDrawerItem(getTranslated(context, 'CUSTOM')!,
            'assets/images/pro_th.svg'),
        CUR_USERID == "" || CUR_USERID == null
            ? Container()
...
/*  _getDivider() {
    ...
    return Card(
      ...
      child: ListTile(
        ...
        onTap: () {
          ...
          }else if (title == getTranslated(context, 'CUSTOM')) {
            Navigator.push(
                context,
                MaterialPageRoute(
  ...
  Widget setMobileField(String mobile) => Padding(
    padding: const EdgeInsets.symmetric(vertical: 8.0, horizontal: 15.0),
    child: Container(
      padding:
      EdgeInsets.only(bottom: MediaQuery.of(context).viewInsets.bottom),
  ...
  Future<bool> validateAndSave(GlobalKey<FormState> key) async {
    ...
    if (form.validate()) {
      if (key == _changePwdKey) {
        ...
        setUpdateUser(
            CUR_USERID!, "", "", nameController.text, emailController.text,mobileController.text);
      }
      return true;
    }
    return false;
